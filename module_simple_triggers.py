   ...
   [
      ...
      (try_for_range, ":center_no", centers_begin, centers_end),
        (try_begin),
          ...
          (val_div, ":cur_rents", 60), ## Tocan ## was 120 #Prosperity of 100 gives the default values
...
## Tocan Invasion+ ##		   
         (else_try),
           # (eq, ":cur_faction", "fac_dark_knights"), ##Tocan Invasion #Do nothing 
...
#         (display_message, "@Village farmers created at {s1}."),
 ...
 # Updating player odds
  ...
  (8,
   [
       (try_for_parties, ":party_no"),
	    (try_begin), ## Tocan   Added after trigger error from the seatrading script
...
## Tocan  
     (call_script, "script_update_tavern_itemsellers"),
     (call_script, "script_update_tavern_trooprecruiters"),	 
#
...
## Tocan+ ##  		 
       (else_try),
         (eq, "$g_player_reading_book", "itm_book_flintlock"),
         (troop_raise_proficiency_linear, "trp_player",wpt_firearm,140),
         (str_store_string, s2, "@ You are now very skilled with pistols."),
       (else_try),
  ...
  (24 * 7,
   [(try_for_range, ":cur_village", villages_begin, villages_end),
      ...
      (party_set_slot, ":cur_village", slot_center_player_relation, ":cur_relation"),
	  		(display_message, "@School improved the relation with your village/s."),	 
    (try_end),
    ]),
...
(4,
   ...
   ##diplomacy end+
	  ...
	  # (display_message, "@{!}DEBUG - DKs DEFEATED"), 
	  (assign, ":continue", 1),
	  (else_try), 
	  (assign, ":continue", 0),
	  # (display_message, "@{!}DEBUG - DKs NOT DEFEATED"), 
	  (try_end), 
	  (try_end),
	  # NE end 
...
#           (try_end),
 ...
 # reserved for future use. For backward compatibility, we need to use these triggers instead of creating new ones.
  ...
  (24,
   ...
   (troop_set_slot, "trp_knight_4_8", slot_troop_home, "p_castle_32"), #Knudarr_Castle   
   #Also the primary six towns (mods not using standard NPCs or map may wish to remove this)
   (troop_set_slot, "trp_kingdom_1_lord", slot_troop_home, "p_town_6"),#Queen Katilus to Praven
   ...
   (try_for_range, ":troop_no", active_npcs_begin, active_npcs_end),
		...
		(this_or_next|eq, ":troop_no", "trp_knight_4_8"),#Knudarr		
  ...
  (24,
   [
	...
	(try_for_range, ":stack_no", 0, ":range"),
		...
		(try_for_range, ":hero_troop", "trp_swadian_hero1", ":end"),
...
# Pay slave labor earnings
   ...
   [
    (try_begin),
     ...
     (assign, "$g_earnings", 0),
      (try_for_range, ":center_no", "p_salt_mine", "p_four_ways_inn"),
...
# Add salt to Barezan's Inventory	
   (24 * 7,
   [
    (try_begin),
		(troop_clear_inventory, "trp_salt_mine_merchant"),
        (troop_add_items,"trp_salt_mine_merchant", "itm_salt",20),
    (try_end),
    ]),  		
## Tocan  end
...
#      (play_sound,"snd_quest_cancelled"),
	 ...
	 (store_random_in_range,":rand",0,5),
	  ...
	  (troop_set_name, "trp_dark_knight_lord",  "@Queen Larktin"),
	  (troop_set_slot, "trp_dark_knight_lord", slot_troop_age, 27),	
	  ...
	  (troop_set_name, "trp_dark_knight_lord1",  "@Lord Purin"),
	  (troop_set_slot, "trp_dark_knight_lord1", slot_troop_age, 37),
	  ...
	  (troop_set_name, "trp_dark_knight_lord2",  "@Lord Grife"),
	  ...
	  (troop_set_name, "trp_dark_knight_lord3",  "@Lord Tantius"),
	  ...
	  (troop_set_name, "trp_dark_knight_lord4",  "@Lord Gracius"),
	  ...
	  (troop_set_name, "trp_dark_knight_lord5",  "@Lord Dionisius"),
	  ...
	  (troop_set_name, "trp_dark_knight_lord6",  "@Lord Chrysogon"),
	  ...
	  (troop_set_name, "trp_dark_knight_lord7",  "@Lord Basyle"),
	  ...
	  (troop_set_name, "trp_dark_knight_lord8",  "@Lord Paugard"),
	  ...
	  (troop_set_name, "trp_dark_knight_lord9",  "@Lord Hardawk"),
	  ...
	  (troop_set_name, "trp_dark_knight_lord10",  "@Lord Americus"),
	  ...
	  (troop_set_name, "trp_dark_knight_lord11",  "@Lord Philcox"),
	  ...
	  (troop_set_name, "trp_dark_knight_lord12",  "@Lord Georgus"),
	  ...
	  (troop_set_name, "trp_dark_knight_lord13",  "@Lord Cyriack"),
	  ...
	  (troop_set_name, "trp_dark_knight_lord14",  "@Lord Sanders"),
	  ...
	  (troop_set_name, "trp_dark_knight_lord15",  "@Lord Alistair"),
	  ...
	  (troop_set_name, "trp_dark_knight_lord16",  "@Lord Hector"),
	  ...
	  (troop_set_name, "trp_dark_knight_lord17",  "@Lord Jeffroy"),
	  ...
	  (troop_set_name, "trp_dark_knight_lord18",  "@Lord Ancelot"),
	  ...
	  (troop_set_name, "trp_dark_knight_lord19",  "@Lord Elshender"),
	  ...
	  (troop_set_name, "trp_dark_knight_lord20",  "@Lord Rawkin"),
...
## Are the Dark Knights a threat yet?
   (24,
    [
      ...
      (display_message,"@The leaders of Calradia have realized the threat the {s1} represent and have unanimously declared war on them."),
      (assign, "$g_recalculate_ais", 1),
   ]),
...
(24 * 7,
	...
	(try_for_range, ":center_no", centers_begin, centers_end),
		...
		(display_message, "@Library improved your renown and the relation with the town/s."),
	(try_end),			
	]),
...
(24 * 7,
	[	
		(faction_set_note_available, "fac_commoners", 0), ## Tocan Bugfix
		(faction_set_note_available, "fac_neutral", 0), ## Tocan Bugfix  	 		
	]),	
	
## Tocan- ##
## Tocan+ ##
   ## Give the player a gift for being a good little vassal
   (24,
    [
      (eq, "$vassal_gift", 0),
      (gt, "$players_kingdom", 0),
      (gt, "$player_has_homage", 0),
      (store_free_inventory_capacity, ":free_inventory"),
      (ge,":free_inventory",1),
      (store_relation, ":relation", "$players_kingdom", "fac_player_faction"),
      (ge,":relation",0),
      (assign,":gift",0),
      (try_begin),
        (eq,"$players_kingdom","fac_kingdom_1"),
        (assign,":gift","itm_plate_armor_swadia"),
      (else_try),
        (eq,"$players_kingdom","fac_kingdom_2"),
        (assign,":gift","itm_vaegir_elite_armor"),
      (else_try),
        (eq,"$players_kingdom","fac_kingdom_3"),
        (assign,":gift","itm_khergit_elite_armor"),
      (else_try),
        (eq,"$players_kingdom","fac_kingdom_4"),
        (assign,":gift","itm_plate_armor_nord"),
      (else_try),
        (eq,"$players_kingdom","fac_kingdom_5"),
        (assign,":gift","itm_plate_armor_rhodok"),
      (else_try),
        (eq,"$players_kingdom","fac_kingdom_6"),
        (assign,":gift","itm_sarranid_elite_armor"),
      (try_end),
      (gt,":gift",0),
      (assign,"$vassal_gift",1),
      (call_script, "script_add_notification_menu", "mnu_notification_give_vassal_gift", ":gift", 0),
   ]),
## Tocan- ##
